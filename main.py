...
def menu_seccion(datos, profesor, seccion):
    """Menú para trabajar con una sección. Todas las operaciones usan esta sección."""
    while True:
        print("\n==========", seccion["nombre"], "==========")
        ...
        if op == "1":
            ...
            if eliminar_seccion(datos, seccion, profesor):
                return
        elif op == "0":
            return
        else:
...
def menu_mis_secciones(datos, profesor):
    """Muestra las secciones y permite elegir una."""
    while True:
        ...
        if len(secciones) == 0:
            ...
            print(len(secciones) + 1, ". Crear sección")
            print("0. Retroceder")
        op = input("Opción: ").strip()
        if op == "0":
            return
        ...
        if 1 <= num <= len(secciones):
            menu_seccion(datos, profesor, secciones[num - 1])
